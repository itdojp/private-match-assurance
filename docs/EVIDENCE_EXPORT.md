...
Public validation strictly loads and Schema-checks the manifest-bound catalog,
requires exactly one entry for the visible fixture ID, and requires that
entry's `test-only` status and candidate digest to match the bundle. The fixture
ID, catalog digest, and candidate digest are therefore one binding rather than
independent labels.
...
Each retained status must also match the corresponding
`review_requirements` and `sanitization_report` marker. Privacy and
security-boundary are always `approved`; IP and vulnerability may be
`approved` or `not-applicable`. A detached bundle digest does not authorize a
caller to rewrite any one of these three status surfaces independently.
